...
--}}
{{-- Esc solo cierra el diálogo que está abierto: si no, cada diálogo de la pantalla mandaría una
     petición a Livewire con cada Esc, aunque estuviese cerrado (B15). --}}
<div x-cloak x-show="{{ $show }}" x-on:keydown.escape.window="if ({{ $show }}) { {{ $show }} = false }" class="fixed inset-0 z-50 flex items-end justify-center p-4 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="{{ $attributes->get('id', 'dialog') }}-title">
